...
// stripK8sPrefix strips the 'k8s:' prefix in the labels generated by
...
// kubernetes does not allow ':' in the name of the label.
func stripK8sPrefix(old map[string]string) map[string]string {
...
func (c *crdBackend) AllocateID(ctx context.Context, id idpool.ID, key allocator.AllocatorKey) error {
	identity := &v2.CiliumIdentity{
		ObjectMeta: metav1.ObjectMeta{
			Name:   id.String(),
			Labels: stripK8sPrefix(key.GetAsMap()),
		},
...
func (c *crdBackend) UpdateKey(ctx context.Context, id idpool.ID, key allocator.AllocatorKey, reliablyMissing bool) error {
	...
	if reliablyMissing {
		...
		if err = c.AllocateID(ctx, id, key); err != nil {
			return fmt.Errorf("Unable recreate missing CRD identity \"%s\"->\"%s\": %s", key, id, err)
		}
	}
...
// GetIfLocked simply wraps Get, and does not respect lock.
