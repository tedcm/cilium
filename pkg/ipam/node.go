...
func (n *Node) abortNoLongerExcessIPs(excessMap *map[string]bool) {
	n.mutex.Lock()
	defer n.mutex.Unlock()
	if n.resource.Status.IPAM.ReleaseIPs == nil || len(n.resource.Status.IPAM.ReleaseIPs) == 0 {
		return
	}
	for ip, status := range n.resource.Status.IPAM.ReleaseIPs {
		if excessMap != nil && (*excessMap)[ip] {
			continue
		}
...
// If exisitng interfaces don't have enough capacity, new interface would be created.
