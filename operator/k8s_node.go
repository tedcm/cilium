		...
		controller.ControllerParams{
			...
			DoFunc: func(ctx context.Context) error {
				...
				for {
					...
					if clusterwide {
						ccnpList, err := ciliumK8sClient.CiliumV2().CiliumClusterwideNetworkPolicies().List(
							meta_v1.ListOptions{
								Limit:    10,
								Continue: continueID,
							})
						...
						cnpList, err := ciliumK8sClient.CiliumV2().CiliumNetworkPolicies(core_v1.NamespaceAll).List(
							meta_v1.ListOptions{
								Limit:    10,
								Continue: continueID,
							})
...
func updateCNP(ciliumClient v2.CiliumV2Interface, cnp *cilium_v2.CiliumNetworkPolicy, nodesToDelete map[string]cilium_v2.Timestamp, capabilities k8sversion.ServerCapabilities) {
	...
	case capabilities.Patch:
		...
		for {
			...
			if ns == "" {
				_, err = ciliumClient.CiliumClusterwideNetworkPolicies().Patch(
					cnp.GetName(), types.JSONPatchType, removeStatusNodeJSON, "status")
			} else {
				_, err = ciliumClient.CiliumNetworkPolicies(ns).Patch(
					cnp.GetName(), types.JSONPatchType, removeStatusNodeJSON, "status")
			}
			if err != nil {
